/**
 * Sidecar client — runs in the AEGIS Node process, spawns the Bare-runtime
 * sidecar (sidecar.cjs) once on first use, multiplexes JSON-RPC requests
 * over stdin/stdout.
 *
 * Why: QVAC native bindings call `require.addon()` which only exists on
 * the Bare runtime. AEGIS's Node host can't load them directly. The
 ...
 *   - Process death is surfaced as a typed error to all in-flight requests.
 *   - shutdown() closes stdin and waits for natural exit (no SIGTERM).
 */
...
const log = createLogger('qvac-sidecar');
const require_ = createRequire(import.meta.url);

const __dirname = dirname(fileURLToPath(import.meta.url));
const SIDECAR_SCRIPT = join(__dirname, 'sidecar.cjs');

// pnpm hoists bins to node_modules/.bin/bare. Resolve via require so we
// don't hard-code a path that breaks on flat npm installs.
function resolveBareBinary() {
  // Try the bare-runtime npm package — it exposes the binary via a per-arch
  // sub-package and ships a launcher at <bare-runtime>/bin/bare.
  try {
    const pkgPath = require_.resolve('bare-runtime/package.json');
    const candidate = join(dirname(pkgPath), 'bin', 'bare');
    if (existsSync(candidate)) return candidate;
  } catch {}
  // Fall back to the hoisted .bin shim that pnpm/npm create.
    ...
    this._spawnPromise = (async () => {
      ...
      if (!bin) {
        throw new SidecarUnavailableError(
          `bare-runtime binary not found. Run "pnpm install" to fetch it ` +
          `(bare-runtime is a real dependency; no shims).`
        );
      }
      ...
      // Wait for {event: 'ready'} before considering the sidecar usable.
    ...
    return await new Promise((resolve, reject) => {
      const onAbort = () => {
        if (this._pending.delete(id)) {
          ...
          // Best-effort cancel; the sidecar's llm-cancel op is request-shaped too.
